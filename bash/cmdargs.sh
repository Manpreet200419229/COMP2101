...
-h )
   echo ' Now Add "-h" for help.'
   echo ' Now process the "-h".'
   echo " Now choose -v for Varbose mode and -d for Debug mode and choose from 1 to 7."
   ;;
-v )
   echo 'Now add "-v" for varbose.'
   echo ' Now process the "-v".'
   verbosevar=1
    ;;
   -d )
   ...
   case "$3" in
   [1-7] )
    echo "Added the -d for debug level $3."
    debugvar=$3
    shift
    ;;
    *)
    echo "Error: The debug mode should be between [1-7]. "
    shift
    esac
    ;;
  *)
